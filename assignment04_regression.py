...
def plot_scatter_with_regression(
    ...
    # Scatter plot
    ax.scatter(df_plot[x_var], df_plot["ret"], alpha=0.5, s=30)
    
    # Overlay regression line
    x_range = np.linspace(df_plot[x_var].min(), df_plot[x_var].max(), 100)
    ...
    x_low, x_high = np.percentile(df_plot[x_var], [2, 98])
    y_low, y_high = np.percentile(df_plot["ret"], [2, 98])
...
def print_key_results(model, x_var: str) -> None:
    ...
    print(f"\nCoefficients:")
    print(f"  Intercept (β₀): {intercept:12.6f}  (SE: {intercept_se:10.6f})")
    print(f"                  t-stat: {intercept_tstat:10.4f}  p-value: {intercept_pval:.4f}")
    print(f"  Slope (β₁):     {slope:12.6f}  (SE: {slope_se:10.6f})")
    print(f"                  t-stat: {slope_tstat:10.4f}  p-value: {slope_pval:.4f}")
    ...
    # Interpretation of slope significance
    sig_level = 0.05
    is_significant = slope_pval < sig_level
    direction = "positive" if slope > 0 else "negative"
    sig_text = "significant" if is_significant else "not significant"
    print(f"\nSlope is {direction} and {sig_text} at the 5% level.")
